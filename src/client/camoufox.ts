...
export class CamoufoxGateway implements BrowserGateway, GreenweezOnboardingGateway {
  ...
  private async waitForDocument(tabId: string): Promise<void> {
  ...
  private async open(url: URL): Promise<string> {
    ...
    try {
      ...
      if (error instanceof ContractChangedError) throw error;
      ...
      if (!/maximum tabs/i.test(error instanceof Error ? error.message : String(error))) throw error;
      const listed = await this.request(`/tabs?userId=${encodeURIComponent(this.userId)}`);
      const tabs = Array.isArray(listed.tabs) ? (listed.tabs as JsonRecord[]) : [];
